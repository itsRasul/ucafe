...
function provider() { return new SmsIrSmsProvider(new ConfigService({ SMSIR_API_KEY: "secret-key", SMSIR_OTP_TEMPLATE_ID: "100000" })); }
...
test("sms.ir verify sends named template parameters and clamps them to 25 characters", async () => {
  ...
  try {
    const result = await provider().sendTemplate({ phone: "09120000000", templateId: 100001, parameters: [{ name: "cafeName", value: "ک".repeat(40) }, { name: "date", value: "1405/06/06" }, { name: "time", value: "18:30" }] });
    ...
    assert.deepEqual(body.parameters.map((parameter) => parameter.name), ["cafeName", "date", "time"]);
