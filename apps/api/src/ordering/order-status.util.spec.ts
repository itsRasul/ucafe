...
test("pickup orders skip courier-only status", () => {
  assert.deepEqual(nextOrderStatuses(OrderStatus.Ready, OrderDeliveryMethod.Pickup), [OrderStatus.Delivered, OrderStatus.Canceled]);
});

test("courier orders move through delivery status", () => {
  assert.deepEqual(nextOrderStatuses(OrderStatus.Ready, OrderDeliveryMethod.Courier), [OrderStatus.OutForDelivery, OrderStatus.Canceled]);
...
test("delivered orders are terminal", () => {
  ...
  assert.deepEqual(nextOrderStatuses(OrderStatus.Canceled, OrderDeliveryMethod.Pickup), []);
});
