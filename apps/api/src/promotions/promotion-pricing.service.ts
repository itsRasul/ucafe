...
export type PricingContext = { product: Map<string, PricingPromotion[]>; category: Map<string, PricingPromotion[]> };
...
export class PromotionPricingService {
  async loadContext(manager: EntityManager, coffeeShopId: string, now: Date): Promise<PricingContext> {
    const promotions = await manager.find(Promotion, { where: { coffeeShopId, isActive: true, deletedAt: IsNull() }, relations: { targets: true } });
    const context: PricingContext = { product: new Map(), category: new Map() };
