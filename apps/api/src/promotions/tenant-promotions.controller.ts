...
export class TenantPromotionsController {
  ...
  list(@Req() request: TenantContextRequest) { return this.promotions.list(this.tenantId(request)); }

  @Get(":id") @RequireTenantPermissions(TenantPermissions.MenuRead)
  get(@Req() request: TenantContextRequest, @Param("id", ParseUUIDPipe) id: string) { return this.promotions.get(this.tenantId(request), id); }

  @Post() @RequireTenantPermissions(TenantPermissions.MenuManage)
  create(@Req() request: TenantAdminRequest, @Body() input: CreatePromotionDto) { return this.promotions.create(this.tenantId(request), request[AUTH_PRINCIPAL]!.userId, input); }

  @Patch(":id") @RequireTenantPermissions(TenantPermissions.MenuManage)
  update(@Req() request: TenantContextRequest, @Param("id", ParseUUIDPipe) id: string, @Body() input: UpdatePromotionDto) { return this.promotions.update(this.tenantId(request), id, input); }

  @Post(":id/activate") @RequireTenantPermissions(TenantPermissions.MenuManage)
  activate(@Req() request: TenantContextRequest, @Param("id", ParseUUIDPipe) id: string) { return this.promotions.setActive(this.tenantId(request), id, true); }

  @Post(":id/deactivate") @RequireTenantPermissions(TenantPermissions.MenuManage)
  deactivate(@Req() request: TenantContextRequest, @Param("id", ParseUUIDPipe) id: string) { return this.promotions.setActive(this.tenantId(request), id, false); }
