...
import { AvailabilityQueryDto, ClientReservationsQueryDto, CreateReservationDto } from "./dto/reservation.dto";
...
export class PublicReservationsController {
  ...
  @Get("mine") @UseGuards(ClientAccessTokenGuard) mine(@Req() req: TenantContextRequest & ClientAuthorizedRequest, @Query() query: ClientReservationsQueryDto) { return this.reservations.mine(req[TENANT_CONTEXT]!.coffeeShopId, req[CLIENT_PRINCIPAL]!.clientId, query); }
  @Get("mine/:id") @UseGuards(ClientAccessTokenGuard) detail(@Req() req: TenantContextRequest & ClientAuthorizedRequest, @Param("id", ParseUUIDPipe) id: string) { return this.reservations.clientDetail(req[TENANT_CONTEXT]!.coffeeShopId, req[CLIENT_PRINCIPAL]!.clientId, id); }
}
