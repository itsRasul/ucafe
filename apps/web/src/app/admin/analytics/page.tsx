...
export default function AnalyticsPage() {
  ...
  const [view, setView] = useState<"overview" | "time" | "products" | "variance">("overview");
  ...
  return <section className="analytics-page">
    <header className="analytics-heading"><div><h1>آمار و تحلیل</h1><p>فروش، سفارش‌ها و اختلاف فیزیکی موجودی کافه.</p></div><span>گزارش‌های عملیاتی</span></header>
    ...
    <nav className="analytics-tabs" aria-label="بخش‌های آمار"><button type="button" aria-current={view === "overview" ? "page" : undefined} onClick={() => setView("overview")}>نمای کلی</button><button type="button" aria-current={view === "time" ? "page" : undefined} onClick={() => setView("time")}>تحلیل زمانی</button><button type="button" aria-current={view === "products" ? "page" : undefined} onClick={() => setView("products")}>محصولات و منو</button>{inventoryPermitted && <button type="button" aria-current={view === "variance" ? "page" : undefined} onClick={() => setView("variance")}>اختلاف مصرف موجودی</button>}</nav>
