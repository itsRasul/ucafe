...
type Item = { id: string; name: string; dimension: string; baseUnit: string; isActive: boolean; batchTrackingEnabled?: boolean };
...
export type PhaseFiveStock = {
  ...
  batchTrackingEnabled?: boolean; batchQuantity?: string; unallocatedQuantity?: string;
...
type WasteLine = { id?: string; itemId: string; itemName?: string; batchId?: string | null; batchNumber?: string | null; quantity: string; unit: string; quantityBase?: string; movementId?: string | null; totalCostToman?: string | null };
...
type WasteForm = { locationId: string; reason: string; wastedAt: string; note: string; items: Array<{ inventoryItemId: string; batchId?: string; quantity: string; unit: string }> };
type WasteBatch = { id: string; batchNumber: string; expiryDate: string | null; remainingQuantityBase: string; status: string };
type Alert = { id: string; itemId: string; itemName: string; baseUnit: string; unit: string; locationId: string; locationName: string; batchId: string | null; batchNumber: string | null; expiryDate: string | null; batchRemainingQuantity: string | null; type: string; status: string; openedAt: string; resolvedAt: string | null; quantity: string; minimumQuantity: string | null; parQuantity: string | null };
...
const alertNames: Record<string, string> = { NEGATIVE: "موجودی منفی", OUT_OF_STOCK: "ناموجود", LOW_STOCK: "موجودی کم", BATCH_EXPIRING_SOON: "در آستانه انقضا", BATCH_EXPIRED: "منقضی شده" };
...
export function WastePanel({ items, locations, canManage, changed }: { items: Item[]; locations: Location[]; canManage: boolean; changed: () => void }) {
  ...
  const [batchesByItem, setBatchesByItem] = useState<Record<string, WasteBatch[]>>({});
  ...
  useEffect(() => {
    let cancelled = false;
    const ids = itemKey ? itemKey.split(",").filter((id) => items.find((item) => item.id === id)?.batchTrackingEnabled) : [];
    if (!form?.locationId || !ids.length) { setBatchesByItem({}); return; }
    void Promise.all(ids.map(async (id) => { const result = await api<{ batches: WasteBatch[] }>(`/tenant/inventory/items/${id}/batches?locationId=${form.locationId}`); return [id, result.batches] as const; }))
      .then((rows) => { if (!cancelled) setBatchesByItem(Object.fromEntries(rows)); }).catch(() => { if (!cancelled) setBatchesByItem({}); });
    return () => { cancelled = true; };
  }, [api, form?.locationId, itemKey, items]);
  ...
  async function editRecord(record: WasteRecord) {
    try {
      ...
      setForm({ locationId: detail.locationId, reason: detail.reason, wastedAt: localInputDate(detail.wastedAt), note: detail.note ?? "", items: (detail.items ?? []).map((line) => ({ inventoryItemId: line.itemId, batchId: line.batchId ?? undefined, quantity: line.quantity, unit: line.unit })) });
  ...
  function warning(line: WasteForm["items"][number]) {
    ...
    const selectedBatch = line.batchId ? batchesByItem[line.inventoryItemId]?.find((batch) => batch.id === line.batchId) : null;
    const stock = micros(selectedBatch?.remainingQuantityBase ?? current), amount = inputBaseMicros(line.quantity, line.unit, item.baseUnit);
...
export function StockAlertsPanel({ items, locations }: { items: Item[]; locations: Location[] }) {
  const { api } = useAdminSession();
  const [status, setStatus] = useState("OPEN"); const [itemId, setItemId] = useState(""); const [locationId, setLocationId] = useState(""); const [type, setType] = useState("");
  const [page, setPage] = useState<Page<Alert>>({ items: [], page: 1, limit: 50, total: 0 }); const [error, setError] = useState("");
  const load = useCallback(async () => {
    const query = new URLSearchParams({ status, limit: "50", page: String(page.page) }); if (itemId) query.set("itemId", itemId); if (locationId) query.set("locationId", locationId); if (type) query.set("type", type);
  ...
  }, [api, status, itemId, locationId, type, page.page]);
  useEffect(() => { void load(); }, [load]);
  return <section className="inventory-panel"><div className="inventory-panel-head"><div><h2>هشدارهای موجودی و انقضا</h2><p>هشدارهای سری موجودی تا مصرف کامل یا اصلاح تاریخ انقضا باز می‌مانند.</p></div></div><div className="inventory-filters"><select value={status} onChange={(event) => { setStatus(event.target.value); setPage((current) => ({ ...current, page: 1 })); }}><option value="OPEN">باز</option><option value="RESOLVED">رفع‌شده</option><option value="ALL">همه</option></select><select value={type} onChange={(event) => { setType(event.target.value); setPage((current) => ({ ...current, page: 1 })); }}><option value="">همه هشدارها</option><option value="NEGATIVE">موجودی منفی</option><option value="OUT_OF_STOCK">ناموجود</option><option value="LOW_STOCK">موجودی کم</option><option value="BATCH_EXPIRING_SOON">در آستانه انقضا</option><option value="BATCH_EXPIRED">منقضی شده</option></select><select value={itemId} onChange={(event) => { setItemId(event.target.value); setPage((current) => ({ ...current, page: 1 })); }}><option value="">همه کالاها</option>{items.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={locationId} onChange={(event) => { setLocationId(event.target.value); setPage((current) => ({ ...current, page: 1 })); }}><option value="">همه محل‌ها</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select></div>{error && <p className="inventory-alert" role="alert">{error}</p>}
    <div className="inventory-table-scroll" tabIndex={0}><table className="inventory-table"><thead><tr><th>کالا / سری</th><th>محل</th><th>وضعیت</th><th>موجودی</th><th>حداقل / انقضا</th><th>زمان بازشدن</th><th>رفع‌شدن</th></tr></thead><tbody>{page.items.map((alert) => <tr key={alert.id}><td><strong>{alert.itemName}</strong>{alert.batchNumber && <small>{alert.batchNumber}</small>}</td><td>{alert.locationName}</td><td><span className={`inventory-status-badge ${alert.status === "OPEN" ? "status-open" : "status-resolved"}`}>{alertNames[alert.type] ?? alert.type}</span></td><td className={alert.type === "NEGATIVE" ? "negative-stock" : ""}>{decimal(alert.batchRemainingQuantity ?? alert.quantity)} {units[alert.unit] ?? alert.unit}</td><td>{alert.expiryDate ? new Date(`${alert.expiryDate}T00:00:00`).toLocaleDateString("fa-IR") : alert.minimumQuantity === null ? "—" : `${decimal(alert.minimumQuantity)} ${units[alert.unit] ?? alert.unit}`}</td><td>{new Date(alert.openedAt).toLocaleString("fa-IR")}</td><td>{alert.resolvedAt ? new Date(alert.resolvedAt).toLocaleString("fa-IR") : "—"}</td></tr>)}{!page.items.length && <tr><td colSpan={7}>هشداری برای این فیلتر وجود ندارد.</td></tr>}</tbody></table></div>
