...
import ReplenishmentAssistant from "./phase-nine";
...
type Item = { id: string; name: string; dimension: "WEIGHT" | "VOLUME" | "COUNT"; baseUnit: string; isActive: boolean; batchTrackingEnabled: boolean; expiryTrackingEnabled: boolean };
...
type ReceiptBatch = { quantity: string; supplierLotNumber?: string; manufacturedDate?: string; expiryDate?: string };
type ReceiptLine = { id: string; purchaseOrderItemId: string | null; inventoryItemId: string; itemName: string; baseUnit: string; locationId: string; locationName: string; quantity: string; unit: string; quantityBase: string; unitPriceToman: string; totalCostToman: string; note: string | null; movementId: string | null; batches: ReceiptBatch[] };
...
type LineForm = { inventoryItemId: string; purchaseOrderItemId?: string; quantity: string; unit: string; unitPriceToman: string; locationId?: string; note?: string; batches?: ReceiptBatch[] };
type Tab = "suppliers" | "orders" | "receipts" | "replenishment";
  ...
  function lineEditor(lines: LineForm[], update: (lines: LineForm[]) => void, receipt = false) {
    return <div className="purchase-lines">{lines.map((line, index) => {
      ...
      return <div className="purchase-line" key={`${line.purchaseOrderItemId ?? line.inventoryItemId}-${index}`}>
        <label>کالا<select required value={line.inventoryItemId} disabled={Boolean(line.purchaseOrderItemId)} onChange={(event) => { const next = [...lines]; const picked = items.find((entry) => entry.id === event.target.value); next[index] = { ...line, inventoryItemId: event.target.value, unit: picked?.baseUnit ?? "piece", batches: receipt && picked?.batchTrackingEnabled ? [{ quantity: line.quantity }] : undefined }; update(next); }}><option value="">انتخاب کالا</option>{items.map((entry) => <option key={entry.id} value={entry.id}>{entry.name} · {unitName[entry.baseUnit]}{entry.batchTrackingEnabled ? " · ردیابی بچ" : ""}</option>)}</select></label>
        <label>مقدار<input required inputMode="decimal" pattern="[0-9]+(\.[0-9]{1,6})?" value={line.quantity} onChange={(event) => { const next = [...lines]; const quantity = event.target.value; next[index] = { ...line, quantity, batches: line.batches?.length === 1 ? [{ ...line.batches[0]!, quantity }] : line.batches }; update(next); }} /></label>
        ...
        {receipt && item?.batchTrackingEnabled && <div className="purchase-batch-lines"><strong>جزئیات سری موجودی</strong>{(line.batches ?? []).map((batch, batchIndex) => <div className="purchase-batch-line" key={batchIndex}><label>مقدار این بچ<input required inputMode="decimal" pattern="[0-9]+(\.[0-9]{1,6})?" value={batch.quantity} onChange={(event) => { const next = [...lines]; const batches = [...(line.batches ?? [])]; batches[batchIndex] = { ...batch, quantity: event.target.value }; next[index] = { ...line, batches }; update(next); }}/></label><label>شماره لات (اختیاری)<input maxLength={100} value={batch.supplierLotNumber ?? ""} onChange={(event) => { const next = [...lines]; const batches = [...(line.batches ?? [])]; batches[batchIndex] = { ...batch, supplierLotNumber: event.target.value }; next[index] = { ...line, batches }; update(next); }}/></label>{item.expiryTrackingEnabled && <label>تاریخ انقضا<input required={item.expiryTrackingEnabled} type="date" value={batch.expiryDate ?? ""} onChange={(event) => { const next = [...lines]; const batches = [...(line.batches ?? [])]; batches[batchIndex] = { ...batch, expiryDate: event.target.value || undefined }; next[index] = { ...line, batches }; update(next); }}/></label>}<label>تاریخ تولید<input type="date" value={batch.manufacturedDate ?? ""} onChange={(event) => { const next = [...lines]; const batches = [...(line.batches ?? [])]; batches[batchIndex] = { ...batch, manufacturedDate: event.target.value || undefined }; next[index] = { ...line, batches }; update(next); }}/></label>{(line.batches?.length ?? 0) > 1 && <button type="button" className="purchase-remove" onClick={() => { const next = [...lines]; next[index] = { ...line, batches: line.batches!.filter((_, row) => row !== batchIndex) }; update(next); }}>حذف بچ</button>}</div>)}<button type="button" className="inventory-secondary" onClick={() => { const next = [...lines]; next[index] = { ...line, batches: [...(line.batches ?? []), { quantity: "0" }] }; update(next); }}>＋ افزودن بچ</button><small>جمع مقدار بچ‌ها باید با مقدار دریافت این ردیف برابر باشد؛ تاریخ انقضا بر اساس روز محلی کافه بررسی می‌شود.</small></div>}
