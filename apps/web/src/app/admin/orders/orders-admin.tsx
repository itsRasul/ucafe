...
type OrderDetail = OrderSummary & { subtotalBeforeDiscountToman: string; discountTotalToman: string; itemDiscountTotalToman: string; orderDiscountToman: string; couponCode: string | null; orderPromotionName: string | null; paymentMethod: "OFFLINE"; deliveryAddressSnapshot: { label: string | null; addressLine: string } | null; customerNote: string | null; nextStatuses: Status[]; items: Array<{ id: string; itemName: string; variantName: string | null; unitPriceToman: string; originalUnitPriceToman: string; discountAmountToman: string; promotionName: string | null; promotionType: string | null; allocationType: string | null; ruleSummary: string | null; quantity: number; lineTotalToman: string }> };
...
export function OrdersAdmin() {
  ...
  useEffect(() => {
    const selectedPromotion = new URLSearchParams(window.location.search).get("promotionId") ?? "";
    setPromotionId(selectedPromotion);
    if (selectedPromotion) { setFromDate(""); setToDate(""); }
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    setSettings(rules);
  }, [api, canManage, canRead, fromDate, promotionId, status, toDate]);
  ...
  return <section className="admin-reservations-shell admin-orders-shell"><header className="admin-page-heading"><div><h1>سفارش‌های آنلاین</h1><p>سفارش‌های مشتریان، روش تحویل و وضعیت آماده‌سازی را مدیریت کنید.</p></div></header>{canManage && <nav className="admin-tabs"><button className={tab === "orders" ? "active" : ""} onClick={() => setTab("orders")}>سفارش‌ها</button><button className={tab === "settings" ? "active" : ""} onClick={() => setTab("settings")}>تنظیمات</button></nav>}{error && <p className="admin-message error" role="alert">{error}</p>}{notice && <p className="admin-message success" role="status">{notice}</p>}{busy && !rows.length ? <div className="admin-inline-loading"><span className="admin-spinner" />در حال بارگذاری سفارش‌ها…</div> : tab === "orders" ? <div className="admin-workspace"><div>{promotionId && <p className="admin-message">نمایش سفارش‌های دارای این تخفیف <button type="button" onClick={() => { setPromotionId(""); const url = new URL(window.location.href); url.searchParams.delete("promotionId"); window.history.replaceState({}, "", url); }}>پاک کردن</button></p>}<form className="reservation-filters" onSubmit={filter}><label>از تاریخ<JalaliDateInput value={fromDate} onChange={setFromDate} /></label><label>تا تاریخ<JalaliDateInput value={toDate} onChange={setToDate} /></label><label>وضعیت<select value={status} onChange={(event) => setStatus(event.target.value as Status | "")}><option value="">همه وضعیت‌ها</option>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button disabled={busy}>اعمال فیلتر</button></form><div className="reservation-list">{rows.map((row) => <button key={row.id} className={selected?.id === row.id ? "selected" : ""} onClick={() => detail(row.id)}><span><strong>{row.client ? `${row.client.firstName} ${row.client.lastName}` : "مشتری"}</strong><small>{formatJalaliDate(row.createdAt.slice(0, 10))} · {deliveryLabels[row.deliveryMethod]}</small></span><span><b className={`status-pill status-${row.status.toLowerCase()}`}>{labels[row.status]}</b><small>{toman(row.totalAmountToman)}</small></span></button>)}{!busy && !rows.length && <div className="admin-empty"><strong>سفارشی پیدا نشد</strong><p>فیلترها را تغییر دهید یا بعداً بررسی کنید.</p></div>}</div></div><aside className="reservation-detail">{selected ? <><div className="detail-title"><div><h2>{selected.client ? `${selected.client.firstName} ${selected.client.lastName}` : "مشتری"}</h2><small dir="ltr">{selected.id}</small></div><b className={`status-pill status-${selected.status.toLowerCase()}`}>{labels[selected.status]}</b></div><dl><div><dt>شماره همراه</dt><dd dir="ltr">{phone(selected.client?.phone)}</dd></div><div><dt>روش تحویل</dt><dd>{deliveryLabels[selected.deliveryMethod]}</dd></div><div><dt>روش پرداخت</dt><dd>پرداخت حضوری</dd></div><div><dt>جمع پیش از تخفیف</dt><dd>{toman(selected.subtotalBeforeDiscountToman)}</dd></div>{Number(selected.itemDiscountTotalToman) > 0 && <div><dt>تخفیف محصولات</dt><dd>−{toman(selected.itemDiscountTotalToman)}</dd></div>}{Number(selected.orderDiscountToman) > 0 && <div><dt>{selected.couponCode ? `کد ${selected.couponCode}` : selected.orderPromotionName ?? "تخفیف سفارش"}</dt><dd>−{toman(selected.orderDiscountToman)}</dd></div>}<div><dt>مبلغ کل</dt><dd>{toman(selected.totalAmountToman)}</dd></div></dl>{selected.deliveryAddressSnapshot && <div className="detail-note"><strong>{selected.deliveryAddressSnapshot.label ?? "نشانی ارسال"}</strong><p>{selected.deliveryAddressSnapshot.addressLine}</p></div>}<div className="order-item-list">{selected.items.map((item) => <article key={item.id}><span><b>{item.itemName}</b><small>{item.variantName ?? "بدون اندازه"}</small></span><span>{fa.format(item.quantity)} × {Number(item.discountAmountToman) > 0 && <del>{toman(item.originalUnitPriceToman)}</del>} {toman(item.unitPriceToman)}{item.promotionName && <small>{item.ruleSummary ?? item.promotionName}</small>}</span><strong>{toman(item.lineTotalToman)}</strong></article>)}</div>{selected.customerNote && <div className="detail-note"><strong>یادداشت مشتری</strong><p>{selected.customerNote}</p></div>}{canManage && <div className="status-actions">{selected.nextStatuses.map((next) => <button key={next} disabled={busy} onClick={() => transition(next)}>{labels[next]}</button>)}</div>}</> : <div className="admin-empty"><strong>یک سفارش را انتخاب کنید</strong><p>جزئیات اینجا نمایش داده می‌شود.</p></div>}</aside></div> : settings ? <form className="reservation-settings" onSubmit={saveSettings}><div className="settings-heading"><div><h2>تنظیمات سفارش آنلاین</h2><p>روش‌های تحویل و پرداخت قابل انتخاب در تسویه.</p></div></div><div className="settings-grid"><label className="toggle"><input type="checkbox" checked={settings.offlinePaymentEnabled} onChange={(event) => setSettings({ ...settings, offlinePaymentEnabled: event.target.checked })} /><span>پرداخت حضوری فعال باشد</span></label><label className="toggle"><input type="checkbox" checked={settings.pickupEnabled} onChange={(event) => setSettings({ ...settings, pickupEnabled: event.target.checked })} /><span>تحویل در کافه فعال باشد</span></label><label className="toggle"><input type="checkbox" checked={settings.courierEnabled} onChange={(event) => setSettings({ ...settings, courierEnabled: event.target.checked })} /><span>تحویل با پیک فعال باشد</span></label></div><button disabled={busy}>{busy ? "در حال ذخیره…" : "ذخیره تنظیمات"}</button></form> : <div className="admin-empty"><strong>سفارش آنلاین برای این پلن فعال نیست</strong><p>برای تغییر تنظیمات، ماژول سفارش آنلاین را روی پلن فعال کنید.</p></div>}</section>;
}
