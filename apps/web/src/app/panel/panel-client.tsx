...
type OrderSummary = { id: string; status: OrderStatus; deliveryMethod: DeliveryMethod; paymentMethod: "OFFLINE"; totalAmountToman: string; subtotalBeforeDiscountToman: string; discountTotalToman: string; itemDiscountTotalToman: string; orderDiscountToman: string; couponCode: string | null; orderPromotionName: string | null; createdAt: string; updatedAt: string };
...
export function PanelOrderDetail({ id }: { id: string }) {
  ...
  return <article className="panel-detail" aria-labelledby="order-detail-title"><Link className="panel-back-link" href="/panel/orders">بازگشت به سفارش‌ها</Link><header className="panel-detail-header"><div><h1 id="order-detail-title">جزئیات سفارش</h1><bdi dir="ltr">{order.id}</bdi></div><Status kind={order.status} label={orderLabels[order.status]} /></header><section className="panel-tracker" aria-labelledby="tracker-title"><h2 id="tracker-title">مسیر سفارش</h2><ol>{stages.map((stage, index) => <li key={stage} className={index < current ? "complete" : index === current ? "current" : ""}><span aria-hidden="true" /><div><strong>{orderLabels[stage]}</strong>{index === current && <small>وضعیت فعلی</small>}</div></li>)}</ol></section><dl className="panel-detail-list"><div><dt>جمع پیش از تخفیف</dt><dd>{formatToman(order.subtotalBeforeDiscountToman)}</dd></div>{Number(order.itemDiscountTotalToman) > 0 && <div><dt>تخفیف محصولات</dt><dd>−{formatToman(order.itemDiscountTotalToman)}</dd></div>}{Number(order.orderDiscountToman) > 0 && <div><dt>{order.couponCode ? `کد ${order.couponCode}` : order.orderPromotionName ?? "تخفیف سفارش"}</dt><dd>−{formatToman(order.orderDiscountToman)}</dd></div>}<div><dt>مبلغ کل</dt><dd>{formatToman(order.totalAmountToman)}</dd></div><div><dt>روش پرداخت</dt><dd>پرداخت حضوری</dd></div><div><dt>روش تحویل</dt><dd>{deliveryLabels[order.deliveryMethod]}</dd></div><div><dt>زمان ثبت</dt><dd>{moment(order.createdAt)}</dd></div><div><dt>آخرین به‌روزرسانی</dt><dd>{moment(order.updatedAt)}</dd></div></dl><section className="panel-detail-section"><h2>اقلام سفارش</h2><ul className="panel-order-items">{order.items.map((item) => <li key={item.id}><div><strong>{item.itemName}</strong>{item.variantName && <span>{item.variantName}</span>}{item.promotionName && <small>{item.ruleSummary ?? item.promotionName}</small>}</div><span>{fa.format(item.quantity)} × {Number(item.discountAmountToman) > 0 && <del>{formatToman(item.originalUnitPriceToman)}</del>} {formatToman(item.unitPriceToman)}</span><b>{formatToman(item.lineTotalToman)}</b></li>)}</ul></section>{address && <section className="panel-detail-section"><h2>{address.label ?? "نشانی ارسال"}</h2><address>{[address.province, address.city, address.addressLine, address.buildingNumber && `پلاک ${address.buildingNumber}`, address.unit && `واحد ${address.unit}`].filter(Boolean).join("، ")}</address>{address.postalCode && <p>کد پستی: <bdi dir="ltr">{address.postalCode}</bdi></p>}</section>}{order.customerNote && <section className="panel-detail-section"><h2>یادداشت شما</h2><p>{order.customerNote}</p></section>}</article>;
}
