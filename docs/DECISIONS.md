...
## D-062 — Inventory variance is count-anchored and historical cost stays unavailable without a close-time basis

- **Status:** implemented on 2026-09-25
- **Decision:** calculate actual depletion from per-item physical count lines plus source-valid trusted stock entries and closing physical count. Calculate theoretical sale use and known waste from the signed immutable movement ledger, validating sale/waste reversal sources. Exclude and flag reconciliation, opening-balance, manual, and invalid-source movements; use `opening counted_at < movement effective time <= closing counted_at`. Preserve a missing count as non-calculable and explicit zero as valid. Require both Inventory and Analytics features and permissions.
- **Reasoning:** ledger sale consumption is expected recipe usage, not an independent physical measure. Current balance average cost is not a historical count-time cost snapshot, and order acceptance time is not retained; substituting present cost or claiming exact order coverage would create false precision.
- **Consequences:** the Persian RTL Analytics report is descriptive/read-only, with signed variance, source drill-down and order-line coverage marked as a created-at proxy. `varianceCostStatus` is unavailable/null until a defensible historical basis exists. The set-based report adds migration `1787872800000`'s movement lookup index but no business tables or corrections. See [INVENTORY.md](INVENTORY.md) and [ANALYTICS.md](ANALYTICS.md).

## D-063 — Inventory batches are movement-backed physical allocations

- **Status:** implemented on 2026-09-25
- **Decision:** make lot tracking optional per item and represent each stock allocation as an existing ledger movement with an optional batch reference. Keep aggregate stock as the canonical item/location balance; batch balances are updated from linked movements, and the difference is the explicit unallocated pool. Allocate automatic order use by FEFO for dated unexpired stock, then FIFO for no-expiry batches; keep expired stock visible and route any shortage to unallocated stock under the existing negative-stock policy. Preserve original batch links on sale and Waste reversals.
- **Reasoning:** this adds lot traceability while keeping existing stock, transactions, source identity, count rebase, and moving-average cost semantics. Historical balances cannot be assigned truthful lot/date data, and physical allocation order is separate from financial valuation.
- **Consequences:** tracking can be enabled on current stock without a backfill; disabling is blocked while positive stock remains. Tracked receipts must fully allocate each receipt line; tracked counts use per-batch plus unallocated rows. Expiry uses `Asia/Tehran` calendar dates, does not auto-dispose stock, and reuses the existing in-app stock-alert table with one open alert per batch. Receipt reversals, transfers, new manual batch creation, expiry push notifications, and historical lot reconstruction remain deferred. Migration `1787876400000-InventoryBatchesAndExpiry` adds tenant-safe batch and movement references, receipt splits, batch-aware counts/waste, alert lifecycle constraints, and reversal validation. See [INVENTORY.md](INVENTORY.md).

## D-064 — Smart purchasing is deterministic PAR assistance based on observed receipts

- **Status:** implemented on 2026-09-25
- **Decision:** reuse the Phase 5 stock projection for committed PO quantities, but calculate an additional Phase 9 replenishment gap after subtracting remaining expired tracked batches from usable stock. Keep the existing operational status and projected stock unchanged. Store optional supplier/item preferred unit, base-normalized MOQ, and one tenant/item preferred supplier as current configuration. Derive price intelligence only from posted Goods Receipt lines, normalize to the item base unit, and compare latest with the immediately previous receipt from the same supplier.
- **Reasoning:** existing ledgers, receipt snapshots, stock rules, unit conversion, and PO lifecycle already own the underlying facts. Keeping the recommendation read-only prevents supplier estimates from changing stock or moving-average cost.
- **Consequences:** a user may explicitly create editable Phase 4 draft POs using an active preferred supplier and the latest recorded receipt price as an estimate. No score, forecast, live price, or autonomous purchasing is implied. Arbitrary package conversions, non-fractional pack rounding, lead time, consumption context, and minimum order value remain unsupported because the current model has no safe source for them. Migration `1787880000000` adds the current supplier/item preference relation. See [INVENTORY.md](INVENTORY.md).

## D-065 — Order analytics preserves created time, terminal outcome time, and explicit source

- **Status:** implemented on 2026-09-25
- **Decision:** count total orders by `created_at`, terminal completion/cancellation by canonical `status_changed_at`, and calculate outcome rates only over `DELIVERED + CANCELED`. Add a nullable typed order source set to `PUBLIC_CLIENT` only by the existing authenticated public checkout; report historical nulls as `UNKNOWN`.
- **Reasoning:** the order row stores current status plus the last transition timestamp, has no rejection state or admin-created cafe orders, and has no website/QR/session attribution. Treating created orders and terminal outcomes as one population or guessing their origin would misstate behavior.
- **Consequences:** canceled value is the immutable submitted order amount, not collected or lost revenue. Source does not identify marketing or entry-point attribution. The order source migration is `1790331236284-AddOrderSource`; Phase 5 reuses Analytics permission, feature entitlement, tenant timezone, and existing timestamp indexes. See [ANALYTICS.md](ANALYTICS.md).

## D-066 — Reservation analytics separates booking, scheduled demand, and terminal outcomes

- **Status:** implemented on 2026-09-25
- **Decision:** count booking volume by `created_at`, scheduled demand by reservation-local date and start time, and current terminal outcomes by `status_changed_at`. Require both effective Analytics and Reservations entitlements. Reserved guests and party-size averages include pending, confirmed, completed, and explicit no-show reservations; canceled and rejected requests are excluded.
- **Reasoning:** creation time and scheduled demand answer different questions. Current-state status cannot establish a canceled reservation's prior confirmation state, so cancellations are excluded from the confirmation-rate denominator; unresolved, canceled, and rejected records do not dilute the completed-versus-no-show rate.
- **Consequences:** completion, cancellation, and rejection rates share the terminal denominator; no-show rate is `NO_SHOW / (COMPLETED + NO_SHOW)`. No-show remains explicit and is never inferred from stale pending state. Capacity history, reservation/order attribution, and creation source remain unavailable from current records. Migration `1790337600000-ReservationAnalyticsIndexes` adds tenant-scoped creation/outcome indexes. See [ANALYTICS.md](ANALYTICS.md).

## D-067 — Product/category promotions share authoritative pricing and immutable order snapshots

- **Status:** implemented on 2026-09-25
- **Decision:** model promotions and product/category targets separately from menu prices. Reuse one server-side calculation for public menu projection, cart quotes, and order creation. Select the largest per-unit discount; break ties by priority, then stable promotion ID. Round percentage savings half up in integer toman and clamp all rewards so they never increase or make a price negative.
- **Reasoning:** catalogue prices remain reusable originals, and customers, checkout, and analytics need consistent net amounts even when promotion state changes later.
- **Consequences:** migration `1790424000000-CreatePromotionsAndOrderSnapshots` adds tenant-safe promotions/targets and backfilled gross/discount totals plus detached order-item reward snapshots. The engine accepts explicit context and typed reward values; generic JSON rule DSLs, stacking, order-level rewards, and marketing conditions remain deferred. See [DISCOUNTS.md](DISCOUNTS.md) and [ANALYTICS.md](ANALYTICS.md).

## D-068 — Coupons activate promotions and offline checkout owns redemption

- **Status:** implemented on 2026-09-25
- **Decision:** keep one promotion pricing service. A tenant-unique uppercase ASCII code activates its promotion at the order stage. Apply item discounts first, then choose the largest of an automatic order reward and a submitted coupon against discounted merchandise; a losing coupon returns a business error. The minimum uses that subtotal, a percentage cap limits the reward, and delivery remains outside discounts. Allow one coupon per order. No exclusive/stackable flags existed in Phase 0, so item and order stages stack while two order-stage rewards do not.
- **Reasoning:** customer orders have only offline payment and no gateway callback or collection ledger. An authenticated client and existing checkout idempotency key provide identity and retry safety. A PostgreSQL coupon row lock and transaction serialize both total and per-customer usage checks without Redis or a separate pricing engine.
- **Consequences:** the order and `APPLIED` redemption commit together. Canceling an `UNDER_REVIEW` order releases its redemption; later cancellation does not automatically restore usage without a financial collection/refund signal. Quote previews consume nothing. Order-level reward and code snapshots preserve history. Multiple coupons, online customer-order payment, exclusive policy, and advanced rewards remain deferred. D-069 adds recurring schedule eligibility without changing redemption semantics. See [DISCOUNTS.md](DISCOUNTS.md).

## D-069 — Weekly schedules are Promotion eligibility in café-local time

- **Status:** implemented on 2026-09-25
- **Decision:** attach zero or more typed weekly time windows to the existing Promotion and run them through the existing eligibility/pricing path for menu, cart, order, and coupon pricing. Evaluate activation from the request timestamp instead of changing owner activation with cron. Use the resolved tenant's `coffee_shops.timezone`; store recurring wall-clock values as PostgreSQL `time` without timezone and canonical weekday keys. A schedule is ANDed with the existing optional UTC `startAt`/`endAt` range. Local start is inclusive and end exclusive. Overnight windows belong to their selected start day; all-day uses an explicit flag. Overlaps are allowed but one Promotion is eligible only once, and exact duplicate windows are rejected.
- **Reasoning:** Happy Hour changes when an existing reward is eligible, not how the reward is computed. Derived eligibility keeps menu and transactional checkout consistent, avoids clock-driven database state and keeps café hours stable across browser/server timezones and DST changes.
- **Consequences:** migration `1790431200000-AddPromotionScheduleWindows` adds tenant-scoped child rows with a composite promotion FK. Coupon validation shares schedule evaluation; existing promotions with no windows remain continuous within their former UTC date range. Order creation recalculates under its existing transaction and immutable order snapshots remain authoritative after creation, including across later schedule changes. Public prices have no shared cache (render memoization is request-scoped), so schedule boundaries require no cache invalidation. Fulfillment-method conditions and public campaign banners remain deferred. See [DISCOUNTS.md](DISCOUNTS.md).

## D-070 — Advanced promotions use typed requirements and deterministic unit allocation

- **Status:** implemented on 2026-09-25
- **Decision:** extend `PromotionPricingService` with typed Buy X Get Y, Bundle, and Quantity Tier rules stored in normalized tenant-scoped child tables. BOGO is a Buy X Get Y preset. Rewards may use only available GET-target cart items already selected by the customer; choose the lowest original unit price, then stable product/variant/unit order, so simple discounts do not shift the reward to a more expensive product. Repeatable Buy/Get and Bundle rules apply only to complete sets; each physical unit can participate in at most one advanced promotion. Quantity chooses the highest qualifying tier and applies the better non-stacked per-unit saving. Bundle savings are distributed proportionally to original item values with any integer remainder assigned by stable product/variant/unit order.
- **Reasoning:** one authoritative pricing path keeps menus, quotes, checkout, coupon/schedule eligibility, and order snapshots consistent. Typed rows validate specific commerce concepts without nullable promotion columns or a generic rule language. Priority-first greedy allocation stays deterministic for café-sized carts without exploring item permutations.
- **Consequences:** migration `1790434800000-AdvancedPromotionRules` adds normalized rule/group/target/tier tables and per-item advanced allocation snapshots. Advanced candidates sort by descending priority then stable promotion ID; completed applications reserve their units. Cart input and target-selection ties use stable product/variant/unit ordering. Each repeated coupon-backed promotion consumes one redemption per order. Order-item quantities remain physical quantities for Inventory, while the core calculation writes no inventory or redemption state. Global cart optimization, automatic reward insertion, custom stacking modes, and modifiers remain deferred. See [DISCOUNTS.md](DISCOUNTS.md).

## D-071 — Customer promotion eligibility uses Clients and Orders, with manual tenant segments

- **Status:** implemented through Discounts & Promotions Phase 4 on 2026-09-26
- **Decision:** add typed customer conditions to the existing Promotion Engine and evaluate them from the authenticated tenant client plus one query-time aggregate over `DELIVERED` orders. Count delivered orders; sum stored net `total_amount_toman`; use `MAX(status_changed_at)` for last qualifying order and the existing client creation time for registration age. First order means a delivered-order count of zero with no active claim for that promotion. Treat all customer conditions on one promotion as AND. Add manual tenant-scoped segments and membership within the Clients domain. Serialize a customer's checkout for first-order claims and persist a claim with each applied first-order reward.
- **Reasoning:** these facts already belong to the stable Clients and Orders domains. Reusing them avoids a second pricing path, duplicated statistics, or an Analytics/CRM dependency. The supported conditions are finite, so typed rows are simpler to validate than a generic rule language.
- **Consequences:** migration `1790500000000-CustomerPromotionEligibility` adds tenant-constrained segments, memberships, conditions, order snapshots, and first-order claims. Archived/inactive segments no longer match but remain referenced by old promotions; they never broaden eligibility. Customer conditions and segment names are snapshotted on applied orders. UCafe still has no CRM module; a future CRM may reuse the Clients-domain segment infrastructure without changing Promotion eligibility. Automated campaigns, scoring, loyalty, and broad CRM features are out of scope. See [DISCOUNTS.md](DISCOUNTS.md).

## D-072 — Promotion analytics consumes immutable commerce snapshots

- **Status:** implemented through Discounts & Promotions Phase 5 on 2026-09-26
- **Decision:** extend the existing Analytics module and UI for promotion reporting. Read tenant-scoped delivered orders and frozen order/item promotion, discount, product/category, coupon, and customer-condition snapshots. Reuse Analytics permissions, feature entitlement, period comparisons, café timezone, and series bucketing. The promotion engine and pricing path do not call Analytics.
- **Reasoning:** immutable allocation snapshots describe what checkout actually charged after later edits, schedule changes, or archival. One analytical usage is a delivered order containing the promotion, independent of repeated BOGO/Bundle sets. A coupon redemption requires an `APPLIED` redemption on a qualifying delivered order. Keep exact discounts attributed to each promotion while deduplicating the order's final value across selected stacked promotions.
- **Consequences:** item gross/net use original item and stored line snapshots. Order-level gross is the value after item discounts and before the order discount; its net is the final stored order value. Stage gross/net can overlap across stacked discounts, so the grouped values are explicitly allocation-stage amounts. Attributed order value is associated sales, not incremental revenue. There is no application count because repeated rule application boundaries are not persisted. Archived records retain snapshot-based reporting; legacy rows without promotion IDs are not reconstructed or backfilled. No schema/index change is needed. Released, canceled, pending, and rejected records do not count. See [ANALYTICS.md](ANALYTICS.md) and [DISCOUNTS.md](DISCOUNTS.md).
