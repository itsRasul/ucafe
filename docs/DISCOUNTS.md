...
Advanced rules extend the same `Promotion` record, eligibility, schedule, coupon, pricing, and checkout flow. Typed tables store one advanced rule per promotion, requirement groups, product/category targets, and quantity tiers. `BUY_X_GET_Y`, `BUNDLE`, and `QUANTITY_TIER` are explicit rule types; they do not create another price engine or alter catalogue prices. Product targets include all variants of the selected product. UCafe does not currently support priced modifiers.

Buy/Get uses one OR target group for each side: a cart must contain the required BUY units and the required GET units. Units are distinct even when both target groups overlap. BUY units are chosen by descending original price, then product/variant ID; reward units use ascending original unit price, then product/variant ID and cart unit index. This keeps the reward on the lowest-priced eligible product even when a simple discount changes its current price. A GET item must already be in the cart and available at checkout; UCafe never adds an unselected product or chooses a random dessert. The reward is applied only when it saves more than the simple per-item reward on that unit. `repeatable` defaults to true and means complete BUY + GET sets repeat until the remaining cart cannot form another full set; false allows one set. A coupon-backed repeated rule still creates one redemption per order.

Bundle groups are ANDed, while products/categories within one group are alternatives. A complete bundle set is paired in group order from the cheapest original-price units, with product ID, variant ID, and cart unit index as stable tie-breakers. Each set applies the configured fixed bundle price only when it is lower than both its original total and its current simple-discounted total. This protects the cafe from an unintended higher category item turning the combo into a larger discount. Repeatable bundles apply to each remaining complete set.

The bundle saving (`original component total − bundle price`) is allocated to components in proportion to their original unit values using integer division. Any remaining toman are assigned in stable product/variant/unit order. Allocated per-unit discounts sum exactly to the bundle discount, and the allocated final prices sum to the bundle price. Separate order-item snapshot rows are written when the same SKU has different unit prices or promotion allocations.

Quantity rules aggregate all eligible available cart units across the selected product/category group. The highest qualifying threshold wins and its reward applies to every eligible unit still available to advanced rules. Percentage and fixed-amount rewards use the existing integer-toman calculator. On an individual unit, the higher of the normal per-item reward and quantity reward wins; rewards do not stack.

The allocation planner starts with Phase 0's best simple per-unit prices, then considers eligible advanced promotions by descending priority and stable promotion ID. A completed advanced application reserves its BUY/GET units, bundle components, or quantity-eligible units so a later advanced rule cannot reuse them. For Get and Quantity rewards, a unit changes to the advanced reward only when it improves the existing saving; a bundle replaces the participating simple discounts only when its aggregate total is better. Order-stage automatic rewards and coupons continue to run after item pricing under Phase 1 rules. This is deterministic priority-first allocation, not a search for the globally largest cart discount.

Cart quote and checkout only submit product/variant IDs and quantities. They reload current menu availability, prices, tenant promotions, schedules, and coupon state. Pricing expands the bounded cart (maximum 50 units) in memory, allocates rewards without writes, then groups units with identical snapshots into order items. A free unit remains quantity 1 at zero payable price, so Inventory still sees the actual fulfilled quantity. Bundle allocations, advanced type, allocation role, reward, and a frozen rule summary are stored on the order-item snapshots. Later promotion edits, schedule changes, or archival do not affect historical order totals. Customer-order payment is still offline; order total remains the authoritative amount.

Reward selection is cart-based rather than a separate selection screen: customers choose the reward product by adding an eligible available item to their cart. If no eligible GET item is present, the rule does not apply. Product/category targets and rule rows are tenant-scoped in service validation and database foreign-key/trigger checks. Advanced calculations are in-memory after tenant-scoped promotion loading; no per-cart-line promotion query or inventory write was added.

Not included: random or automatic reward insertion, cart-wide optimal-packing, configurable stacking modes, paid modifiers, automatic public campaign banners, refunds, and customer segmentation.
