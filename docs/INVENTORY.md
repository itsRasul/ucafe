...
## Phase 8 batches, lot traceability and expiry

Phase 8 is implemented with optional per-item batch and expiry tracking. Every batch belongs to one tenant, item, and location and records its optional supplier lot, manufacture/expiry dates, receipt provenance, original normalized quantity, receipt cost snapshot, and remaining quantity. Batch history comes from the existing immutable movement ledger; a database trigger applies each linked movement to the batch projection and rejects a negative batch balance. Aggregate `StockBalance` remains the canonical item/location quantity. For tracked items, aggregate stock equals all batch balances plus the unallocated pool; that pool is the aggregate balance minus the sum of batch balances and may be positive legacy/manual stock or negative shortage. Existing quantities are not backfilled into invented lots.

New tracked items require opening batch quantities to equal the opening quantity. On an existing item, enabling tracking leaves its current balance unallocated. Batch-tracked receipts must split every line into one or more batches whose normalized base quantities exactly equal the receipt line; direct and PO receipts use the same flow. An item with expiry tracking enabled requires an expiry date on each opening or received batch. Expiry can later be switched off without erasing historical dates. Tracking cannot be disabled while any batch or aggregate positive stock remains. Non-tracked receipts and all prior stock history retain Phase 4 behavior.

Order consumption allocates dated, non-expired batches first by expiry date (FEFO), then batches without expiry by `received_at`, creation time, and ID (FIFO tie-breaks). An expiry date remains valid through that date in the `Asia/Tehran` café timezone. Automatic sale consumption excludes expired batches; if eligible batch stock is insufficient, the existing negative-stock policy posts the shortage to the unallocated pool instead of consuming an expired lot. Manual decreases allocate FEFO/FIFO including expired stock unless a specific batch is selected. A positive manual adjustment without a batch is unallocated; one with a batch increases that existing lot. Each split allocation is a separate movement with a stable batch-specific idempotency key. Sale reversals copy the original movement's batch and cost snapshot, even if that batch has since expired.

Waste lines may identify a batch or unallocated stock. Posting locks item/location balances and selected batches, rejects a batch overdraw, and writes the existing cost-snapshotted `WASTE` movement. Full-record reversal restores each original allocation. Physical counts for tracked items must include an `UNALLOCATED` line and each positive batch; aggregate-only lines are rejected. Reconciliation is per allocation and includes later movements from that same batch or the unallocated pool, preserving the Phase 1 count-rebase behavior.

Batch status is derived (`ACTIVE`, `EXPIRING_SOON`, `EXPIRED`, `NO_EXPIRY`, `DEPLETED`). Expiry compares calendar dates in `Asia/Tehran`; crossing expiry never creates Waste. Item warning days default to three. Expiry alerts reuse `inventory_stock_alerts`, keep one open alert per batch, transition from expiring to expired in place, and resolve when depleted or corrected. The API checks alerts on reads and stock changes and has a lightweight in-process date poll; no separate scheduler or notification channel was added. Overview exposes expiring/expired batch counts and expired quantities grouped by base unit so unlike units are not added together.

`GET /batches`, `GET /batches/:id`, `PATCH /batches/:id`, and `GET /items/:id/batches` provide tenant-scoped lot views, movement history, provenance, and audited metadata corrections. Existing `GET /movements?batchId=...`, count, waste, receipt, stock-alert, and item configuration APIs include batch fields. The Persian RTL `/admin/inventory` surfaces batch balances and alerts, batch detail/audit history, lot entry on receipts, batch-specific waste, and per-batch counts. Migration `1787876400000-InventoryBatchesAndExpiry` adds tenant-safe batch/movement links, receipt batch splits, allocation-aware counts and waste, expiry alerts, provenance guards, and exact-batch reversal checks. `apps/api/src/inventory/order-inventory.spec.ts` exercises multi-batch FEFO, expired-batch exclusion, retry, exact reversal, tenant isolation, expiry alert transition/depletion, overdraw rejection, batch Waste, and count rebasing. `apps/api/src/inventory/purchasing.spec.ts` covers strict receipt batch totals, unit normalization, split cost snapshots, and movement totals.

Physical FEFO/FIFO does not change moving weighted-average financial costing. Batch receipt costs are provenance only; recipe costing and Waste cost snapshots continue to use the established item/location average. Receipt reversal, batch transfers, new manual-batch creation, hard-vs-best-before policy, expiry push notifications, and historical lot reconstruction remain deferred.
...
## Phase 9: smart purchasing and supplier price intelligence

Phase 9 adds a read-only replenishment assistant, factual supplier-price comparisons, current supplier/item purchasing preferences, receipt-price history, and explicit draft PO assistance. It remains inside Inventory and uses the existing `inventory` feature and `inventory.read` / `inventory.manage` permissions.

The assistant reuses Phase 5's paginated stock projection. Committed quantity is the sum of ordered or partially received PO-line quantity less posted receipt quantity, floored at zero; draft, canceled, received, and direct receipts do not add an open commitment. The existing operational status, current quantity, projected quantity, and Phase 5 projected PAR gap retain their prior meanings. For tracked items, the assistant additionally subtracts remaining expired batch quantity when calculating usable stock for the replenishment gap, so expired lots do not suppress a purchase suggestion. The displayed current status is still based on the canonical aggregate balance and minimum/PAR rules. The gap is `max(PAR - (usable current quantity + committed open PO quantity), 0)`; no PAR means no recommendation. Negative current stock is preserved.

Suggested quantity is this deterministic target gap, increased to the configured supplier/item MOQ when the gap is positive. The MOQ is stored as normalized base quantity. Purchase units reuse existing conversion rules: weight (`g`/`kg`), volume (`ml`/`l`), and the configured base unit for count items. Decimal quantities are preserved for convertible units. Arbitrary packages such as a bag, carton, or box with a custom content size are not modeled, so no package rounding is guessed; count-item packaging conversions remain rejected. Minimum order value, lead time, recent consumption, and recent weighted-average supplier price are not implemented.

Current supplier configuration lives in `inventory_supplier_items`: one supplier/item row may store a preferred purchase unit and MOQ, while a partial unique index allows at most one preferred supplier per tenant/item across locations. Preference changes affect only current purchasing assistance and do not rewrite POs or receipt history. Inactive suppliers remain visible in historical comparison, but an inactive preferred supplier receives a warning and is not offered for new draft assistance.

Supplier price truth remains posted Goods Receipt lines, including direct receipts. Draft PO estimates and unposted receipts are excluded. For each supplier/item/receipt, normalized price is the sum of posted line totals divided by the sum of normalized base quantities, rounded to the established six-place base-unit cost precision. This quantity-weights multiple lines for the same item in one receipt, so the immediately previous purchase means the previous receipt, not another line from the same receipt. Supplier comparison shows each tenant supplier, including suppliers without a recorded price; each available latest price is compared with that same supplier/item's immediately previous posted receipt. Change percent is `(latest - previous) / previous × 100`; a missing or zero previous price yields `null`. Price history is paginated and chronological. Dates identify when UCafe last observed a price; they are not live quotes and have no arbitrary stale cutoff.

Estimated replenishment cost uses the latest normalized actual receipt price and the suggested base quantity, rounded half-up to whole toman. Draft PO unit estimates convert that normalized price to the selected existing purchase unit and use the Phase 4 integer-toman PO line field. The assistant creates drafts only after an explicit user action, groups lines by preferred supplier and location, and calls the existing `POST /purchase-orders` flow. Draft values remain editable there. A missing actual price is null and does not enable assistant-created drafts. Drafts and recommendations do not create stock movements or change average cost; posted Goods Receipts remain the inventory/cost trigger.

Routes under `/api/v1/tenant/inventory` add `GET /replenishment`, `GET /items/:itemId/supplier-prices`, `GET /items/:itemId/supplier-price-history?supplierId=...`, and `PUT /items/:itemId/suppliers/:supplierId/preference`. Existing `/purchase-orders` creates the draft. Reads and writes use resolved tenant context, existing RBAC, and the shared Inventory entitlement. Replenishment delegates stock math to the existing paginated stock query and enriches each page with one bulk query; supplier comparisons use a bounded supplier page and a window-ranked receipt query; history is paginated. Migration `1787880000000-InventorySupplierPreferences` adds the preference table, tenant-safe foreign keys, pair uniqueness, item lookup, and preferred-supplier uniqueness. Receipt-history queries reuse existing receipt-line indexes; no unverified receipt index was added.

The Persian RTL assistant is a new tab at `/admin/inventory/purchasing`. It displays current status, quantity, minimum, PAR, committed order quantity, projected stock, target gap, MOQ, preferred supplier, latest normalized price/date, last receipt source, previous-price change, expiry warnings, and estimated cost. It includes supplier comparison, posted receipt history, editable preference settings, page filters, bulk selection, and draft creation. Advanced charts and Analytics spend reports remain out of scope. Historical purchasing BI belongs in Analytics if later implemented; Phase 9 operational calculations require Inventory only.

Verification coverage adds exact integer-math price estimates, DTO validation, inventory feature gating, tenant-scoped preference validation, actual-price normalization/latest/previous comparisons, percent change, paginated receipt history, committed partial PO projection, MOQ application, no stock side effects, and expired-batch exclusion. See the Phase 9 result and any environment limitations in [PROGRESS.md](PROGRESS.md) and decision D-064 in [DECISIONS.md](DECISIONS.md).

Known limits: there is no live supplier API, arbitrary pack-size conversion or integer-pack rounding, supplier-wide minimum order value, supplier scoring, weighted recent average, lead-time metric, consumption context, or advanced spend analytics. Phase 10 should start by defining and validating a dated item/location consumption series from source-valid sale-consumption movements and reversals (with recipe-version context), then evaluate a deterministic baseline before exposing any forecast quantity. It must keep that forecast separate from this phase's PAR replenishment gap.
