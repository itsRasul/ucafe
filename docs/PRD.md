...
- Owners with menu-management permission can run scheduled, tenant-scoped percentage, fixed-amount, and fixed-price promotions on products or categories, plus percentage/fixed-amount entire-order promotions and coupon-activated promotions. Cart quotes and order creation use server-side pricing; orders retain immutable price and promotion snapshots.
...
- Tenant owners with the configurable Analytics plan feature can review delivered-order revenue, time/product/customer analytics, order outcomes, cancellation value, fulfillment, and recorded order sources with local-period comparisons. Order source distinguishes public-client checkout from historical unknown records; delivered value is not verified cash collection. See [ANALYTICS.md](ANALYTICS.md).
