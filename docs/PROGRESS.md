...
- Curated website editor for hero/story, safe theme colors, approved fonts/radius, contact/location and all seven opening-hour rows.
...
- 2026-09-12: Checkout/result UI pass completed. Added tenant-colored checkout, courier address, optional customer note, cart authentication modal polish, and authenticated `/checkout/result` using the existing order endpoint. Web type-check and production build passed; Docker live verification was unavailable because the local Docker pipe denied access.
- 2026-09-12: Courier delivery visibility corrected. The ordering settings entity and migration `1787799600000-EnableCourierDelivery` enable courier delivery for existing settings and future defaults. Checkout already renders the enabled courier method and its address panel. The auth modal no longer changes body overflow and uses a centered, soft backdrop. API and web type-checks passed.
- 2026-09-12: Checkout address picker redesigned. The courier address section now renders as a compact summary box under the courier delivery option, showing the selected address or a prompt, and opens a Persian RTL address modal containing the saved-address list, per-address edit and delete actions, and the add/edit form; the previously unwired address-edit save path is now reachable. Web type-check and production build passed; live browser verification was unavailable because no browser surface is connected to this session.

- 2026-09-12: Tenant admin order list fixed. The /tenant/orders list threw TypeError: Cannot read properties of undefined (reading ''databaseName'') (HTTP 500) because OrderingService.list passed snake_case database column names to the TypeORM QueryBuilder; the skip/take pagination path resolves orderBy via metadata.findColumnWithPropertyPath, which only matches entity property names. The list and locked-status queries now use property paths (order.coffeeShopId, order.createdAt). API type-check passed, the API image was rebuilt, and the exact query (filters + pagination + client join) was re-run against live PostgreSQL, returning the existing tenant orders instead of erroring.

- 2026-09-13: Toast placement fixed. The cart and featured Toastify containers already rendered `position: fixed` with `position=bottom-right`, but the shared `section { animation: enter .5s ease-out both }` rule in `apps/web/src/app/styles.css` left a filling animation on every `<section>`, which permanently makes that section the containing block for `position: fixed` descendants; the toasts were therefore pinned to the bottom-right of the menu/featured section instead of the viewport. The entrance animation now uses fill mode `backwards`, which renders identically and releases the containing block. Web type-check passed, and a headless-Chrome check against the real stylesheets confirmed the toast container lands on the viewport bottom-right (with the 1rem library inset) on both the menu-page and home-page ancestor chains while section geometry is unchanged. The toast markup, props, tenant theme inheritance and the header account toast were untouched.

- 2026-09-13: Hidden toast wrapper layout space removed. React-Toastify renders every container inside an in-flow `<section class=Toastify id={containerId}>`, and the app's global `section` rule styles every section as a padded card, so those wrappers were real layout boxes (320x140 inside `nav.tenant-nav`, 985x140 inside `section.public-menu-content` / `div.reveal`). That pushed the tenant navbar onto a second grid row (141px -> 93px) and added ~140px of empty page height. `apps/web/src/app/tenant.css` now sets `.Toastify { display: contents }`, so the wrappers generate no box at all while the fixed toast containers still resolve against the viewport. Verified in headless Chrome against the live dev server: both wrappers measure 0x0, the navbar is back to 93px (brand row 83px -> 59px), page height drops 2084 -> 1944 on `/menu` and 6029 -> 5889 on the storefront, there is no horizontal overflow, and fixed probes placed inside both wrappers land at the viewport bottom-right. Pause-on-hover is per-toast in this version, so the wrapper losing its box changes no behavior.
- 2026-09-13: Storefront featured cards simplified. `apps/web/src/app/tenant-featured.tsx` no longer renders the add-to-cart/quantity control (or its ordering-unavailable modal and toast container), so the پیشنهاد کافه cards are pure links to `/menu?item=`; the component is now a server component with the cart-store, toast and modal imports deleted, and `TenantStorefront` passes only `items`. The underlines the user saw on the card name, description and price were the browser default on the card's `<a class=public-menu-card-main>` (no `text-decoration` reset existed), so `apps/web/src/app/tenant.css` now sets `text-decoration: none` on that anchor, which also covers the identical menu-page cards. Web type-check and production build passed; the removal of the button was not separately verified in a browser.

- 2026-09-13: Storefront footer now embeds the cafe location map. `TenantFooter` renders a lazy `<iframe>` (Google Maps keyless embed, pin centred on the saved coordinates) spanning a full footer row whenever `contact.latitude` and `contact.longitude` are both set, and renders nothing otherwise. Only `apps/web/src/app/tenant-public.tsx` and the new `.footer-map` rules in `apps/web/src/app/tenant.css` changed; the admin latitude/longitude fields, `site.service.ts` projection and the existing `/#visit` map link were already in place. Web type-check passed.
