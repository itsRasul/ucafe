...
$containerFile = "/tmp/cafexa-$stamp.dump"
$targetFile = Join-Path $targetDirectory "cafexa-$stamp.dump"
try {
  docker compose -f (Join-Path $repositoryRoot "compose.yaml") exec -T postgres pg_dump -U cafexa -d cafexa --format=custom --file=$containerFile
