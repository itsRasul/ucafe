...
$database = "cafexa_restore_verify_$(Get-Date -Format 'yyyyMMddHHmmss')"
if ($database -notmatch '^cafexa_restore_verify_[0-9]{14}$') { throw "Unsafe verification database name" }
...
try {
  ...
  docker compose -f (Join-Path $repositoryRoot "compose.yaml") exec -T postgres createdb -U cafexa $database
  if ($LASTEXITCODE -ne 0) { throw "Creating the verification database failed" }
  docker compose -f (Join-Path $repositoryRoot "compose.yaml") exec -T postgres pg_restore -U cafexa -d $database --exit-on-error $containerFile
  if ($LASTEXITCODE -ne 0) { throw "Restoring the backup failed" }
  $migrationCount = docker compose -f (Join-Path $repositoryRoot "compose.yaml") exec -T postgres psql -U cafexa -d $database -Atc "SELECT count(*) FROM migrations"
  ...
  docker compose -f (Join-Path $repositoryRoot "compose.yaml") exec -T postgres dropdb -U cafexa --if-exists $database | Out-Null
